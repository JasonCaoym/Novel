...
public class ApiHttpWorker {
    ...
    public ApiHttpWorker(ApiRequest req) {
        mRequest = req;
    }
    ...
    private URL createApiUrl() {
        ...
        if (ApiRequest.REQ_METHOD_GET == mRequest.getRequestMethod() && !mRequest.isExternalReq()) {
    ...
    private boolean isValidApiResponse() {
        ...
        try {
            JSONObject jsonObj = new JSONObject(mResponseContent);
            Logger.d(TAG, "url=" + mRequest.getUrl() + "  isLoadFromCache = " + isLoadFromCache + "    response:" + jsonObj.toString(2));
