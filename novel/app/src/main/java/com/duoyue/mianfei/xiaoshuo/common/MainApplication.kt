...
import com.duoyue.mianfei.xiaoshuo.read.utils.Utils.fix
...
@DefaultLifeCycle(
    application = "com.duoyue.mianfei.xiaoshuo.common.SampleApplication",
...
class MainApplication(
    ...
    override fun onCreate() {
        super.onCreate()
        if (PhoneUtil.getCurrentProcessName(application).equals(application.packageName)) {
            SPUtils.initPres(application)
            // 初始化假的BaseApplication
            var baseApplication = BaseApplication(application, this)
            ...
            ZExecutorService.getInstance().execute(object : Runnable {
                override fun run() {
                    ...
                    MtStHelper.visit()
                    //初始化友盟统计.
                    StatisHelper.init(application)
                    //初始化掌阅统计SDK.
                    ZyStatsApi.init(application)
                    //========取消========处理极光推送SDK=================
...
//                    BlockCanary.install(application, AppContext()).start()
    ...
    private fun initApp() {
        ...
        CrashReport.initCrashReport(application, "fd896bd2d7", true)
        ServiceLauncher.init(application)
        //UiThreadBlockWatcher.install(3000, UiThreadBlockWatcher.TYPE_LOOPER);
        //初始化广告数据.
//        AdManager.getInstance().initPlatform(application)
        AdConfigManger.getInstance().init(application)
    ...
    override fun onLowMemory() {
        ...
        GlideUtils.cleanMemory(application)
    }
