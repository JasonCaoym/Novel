 ...
 * 圆圈式的指示器
 */
public class RoundCircleNavigator extends View implements IPagerNavigator {
    private int mRadius;
    ...
    public RoundCircleNavigator(Context context) {
        super(context);
        init(context);
    }
        ...
        switch (mode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                result = mTotalCount * mRadius * 2 + (mTotalCount - 1) * mCircleSpacing
                        + getPaddingLeft() + getPaddingRight() + mStrokeWidth * 2 + mRadius * 6;
                break;
            default:
                break;
        }
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        if (mCirclePoints.size() > 0) {
            canvas.drawRoundRect(mIndicatorX - 3*mRadius, getHeight() / 2 - mRadius / 2 - 2 * mStrokeWidth,
                    mIndicatorX + 3*mRadius, getHeight() / 2 + 2 * mRadius - 2 * mStrokeWidth, (float)mRadius + mStrokeWidth, (float)mRadius + mStrokeWidth, mPaint);
        }
    }
    ...
    private void prepareCirclePoints() {
        ...
        if (mTotalCount > 0) {
            ...
            int startX = 3 * mRadius + (int) (mStrokeWidth / 2.0f + 0.5f) + getPaddingLeft();
