 ...
 * 1-2-2
 */
public class FixedOne2DoubleView extends AbsItemView<BookOne2DoubleBean> implements BookCityPresenter.BookMoreView {
...
//            R.id.fix_row_2,
    ...
    public void onCreate() {
        setContentView(R.layout.book_city_1_4_row_layout);
        //获取页面Id.
    ...
    public void onClick(View view) {
        if (noDoubleListener()) {
            if (mItemData.getBookCityModuleBean().getChildColumns().get(0).getBooks() != null/* && (Integer) view.getTag() <= mItemData.moduleBean.getBooks().size()*/) {
    ...
    public void onSetData(boolean isFirstSetData, boolean isPosChanged, boolean isDataChanged) {
        ...
        for (int index = 0; index < mIds.size(); index++) {
            View view = mItemView.findViewById(mIds.get(index));
            if (index == 0) {
//                if (mItemData.moduleBean.getBooks().size() < 5) {
...
//                textView.setTypeface(typeFace);
                ...
                if (data != null) {
                    ...
                    if (index == 1) {
                        ...
                        TextView mTv_grade = view.findViewById(R.id.tv_grade);
                        mTv_grade.setVisibility(View.VISIBLE);
                        ...
                        mTv_grade.setText(s1);
                        ((TextView) view.findViewById(R.id.book_resume)).setText(data.getResume());
                        view.findViewById(R.id.book_count).setVisibility(View.GONE);
                        ...
                        GlideUtils.INSTANCE.loadImage(mActivity, data.getCover(), (ImageView) view.findViewById(R.id.book_cover), GlideUtils.INSTANCE.getBookRadius(),ViewUtils.dp2px(68), ViewUtils.dp2px(87));
                        ((TextView) view.findViewById(R.id.book_name)).setText(data.getName());

                        ((TextView) view.findViewById(R.id.tv_fen)).setText(String.format("%s分", data.getStar()));

                        if (data.getPopularityNum() >= 100000000) {
                            ((TextView) view.findViewById(R.id.book_popularity)).setText(String.format("%.1f", data.getPopularityNum() * 1f / 100000000) + "亿人在读");
                        } else if (data.getPopularityNum() > 10000) {
                            ((TextView) view.findViewById(R.id.book_popularity)).setText(String.format("%s万人在读", data.getPopularityNum() / 10000));
                        } else {
                            ((TextView) view.findViewById(R.id.book_popularity)).setText(String.format("%s人在读", data.getPopularityNum()));
                        }
