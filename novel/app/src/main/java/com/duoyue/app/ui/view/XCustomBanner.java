...
public class XCustomBanner extends RelativeLayout implements XBannerViewPager.AutoPlayDelegate, ViewPager.OnPageChangeListener {
    ...
    private XCustomBanner.OnItemClickListener mOnItemClickListener;
    ...
    private XCustomBanner.AutoSwitchTask mAutoSwitchTask;
    ...
    private XCustomBanner.XBannerAdapter mAdapter;
    ...
    /**
     * 监听到当前View显示事件时, 是否启动播放.
     */
    private boolean isVisibleStartPaly = true;
     ...
     */
    @Deprecated
    public void setmAdapter(XCustomBanner.XBannerAdapter mAdapter) {
        this.mAdapter = mAdapter;
    }

    public void loadImage(XCustomBanner.XBannerAdapter mAdapter) {
        this.mAdapter = mAdapter;
    }

    public XCustomBanner(Context context) {
        this(context, null);
    }

    public XCustomBanner(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XCustomBanner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        mAutoSwitchTask = new XCustomBanner.AutoSwitchTask(this);
...
//            mTipTv.setGravity(Gravity.CENTER_VERTICAL | Gravity.RIGHT);
    ...
    private void setBannerPlaceholderDrawable() {
        if (mPlaceholderDrawableResId != NO_PLACE_HOLDER && mPlaceholderImg == null) {
            mPlaceholderImg = new CustomImageView(getContext());
...
//            }
    ...
    public void setBannerData(@NonNull List<? extends SimpleBannerInfo> models) {
        setBannerData(R.layout.book_banner_item, models);
    }
     ...
     * @param position
     */
    public void setPointPosition(@XBanner.INDICATOR_GRAVITY int position) {
        //设置指示器布局位置
        if (CENTER == position) {
            mPointRealContainerLp.addRule(RelativeLayout.CENTER_HORIZONTAL);
     ...
     * @param position
     */
    public void setPointContainerPosition(@XBanner.INDICATOR_POSITION int position) {
        if (BOTTOM == position) {
    ...
    private void initViewPager() {
        ...
        mViewPager.setAdapter(new XCustomBanner.XBannerPageAdapter());
...
//            }
            ...
            if (mOnItemClickListener != null && !mDatas.isEmpty()) {
                view.setOnClickListener(new OnDoubleClickListener() {
                    ...
                    public void onNoDoubleClick(View v) {
                        mOnItemClickListener.onItemClick(XCustomBanner.this, mDatas.get(realPosition), v, realPosition);
                    }
                });
            }

            if (null != mAdapter && !mDatas.isEmpty()) {
                mAdapter.loadBanner(XCustomBanner.this, mDatas.get(realPosition), view, realPosition);
            }
    ...
    {
        isVisibleStartPaly = visibleStartPaly;
    }
    ...
    private static class AutoSwitchTask implements Runnable {
        private final WeakReference<XCustomBanner> mXBanner;

        private AutoSwitchTask(XCustomBanner mXBanner) {
            this.mXBanner = new WeakReference<>(mXBanner);
        }

        @Override
        public void run() {
            XCustomBanner banner = mXBanner.get();
            if (banner != null) {
    ...
    public void setOnItemClickListener(XCustomBanner.OnItemClickListener listener) {
        mOnItemClickListener = listener;
    }

    public interface OnItemClickListener {
        void onItemClick(XCustomBanner banner, Object model, View view, int position);
    }

    public interface XBannerAdapter {
        void loadBanner(XCustomBanner banner, Object model, View view, int position);
    }
}
