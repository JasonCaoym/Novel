...
public class CountDownView extends LinearLayout {
    ...
    private AtomicBoolean isContinue = new AtomicBoolean(false);// 是否开启倒计时
    ...
    private void init() {
        ...
        // 添加子控件
        // 小时控件
        int mainColor = getContext().getResources().getColor(R.color.standard_red_main_color_c1);
        hourTv = new TextView(context);
        hourTv.setTextColor(mainColor);
        hourTv.setBackgroundColor(Color.parseColor("#FFF7EA"));
        hourTv.setTextSize(22);
        hourTv.setGravity(Gravity.CENTER);
        hourTv.setIncludeFontPadding(false);
        this.addView(hourTv);
        // 小时冒号控件
        hourColonTv = new TextView(context);
        hourColonTv.setTextColor(Color.parseColor("#FFFFFF"));
        hourColonTv.setTextSize(22);
        hourColonTv.setText(R.string.colon);
        hourColonTv.setGravity(Gravity.CENTER);
        hourColonTv.setIncludeFontPadding(false);
        this.addView(hourColonTv);
        // 分钟控件
        minuteTv = new TextView(context);
        minuteTv.setTextColor(mainColor);
        minuteTv.setBackgroundColor(Color.parseColor("#FFF7EA"));
        minuteTv.setTextSize(22);
        minuteTv.setGravity(Gravity.CENTER);
        minuteTv.setIncludeFontPadding(false);
        this.addView(minuteTv);
        // 分钟冒号控件
        minuteColonTv = new TextView(context);
        minuteColonTv.setTextColor(Color.parseColor("#FFFFFF"));
        minuteColonTv.setTextSize(22);
        minuteColonTv.setText(R.string.colon);
        minuteColonTv.setGravity(Gravity.CENTER);
        minuteColonTv.setIncludeFontPadding(false);
        this.addView(minuteColonTv);
        // 秒控件
        secondTv = new TextView(context);
        secondTv.setTextColor(mainColor);
        secondTv.setBackgroundColor(Color.parseColor("#FFF7EA"));
        secondTv.setTextSize(22);
        secondTv.setGravity(Gravity.CENTER);
        secondTv.setIncludeFontPadding(false);
     ...
     * 设置冒号控件间距
     *
     * @param leftMargin  左边距 0取默认值
     * @param rightMargin 右边距 0取默认值
     * @return CountDownView
     */
    public CountDownView setColonMargin(int leftMargin, int rightMargin) {
        if (leftMargin > 0 && rightMargin > 0) {
            ViewGroup.LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
            ((LayoutParams) params).leftMargin = leftMargin;
            ((LayoutParams) params).rightMargin = rightMargin;
    ...
    private void countDown() {
        ...
        ZExecutorService.getInstance().execute(mRunnable);
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case UPDATE_UI_CODE:// 刷新UI
                    ...
                    // 倒计时结束
                    if (!currentCountDownView.isContinue.get()) {
